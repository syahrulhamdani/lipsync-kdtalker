...
class DenseMotionNetwork(nn.Module):
    ...
    def create_heatmap_representations(self, feature, kp_driving, kp_source):
        ...
        heatmap = (gaussian_driving - gaussian_source).to("mps", dtype=torch.float16)  # (bs, num_kp, d, h, w)

        # adding background feature
        zeros = torch.zeros(heatmap.shape[0], 1, spatial_size[0], spatial_size[1], spatial_size[2]).to("mps")
